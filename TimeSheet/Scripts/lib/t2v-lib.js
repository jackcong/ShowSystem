...
t2v_lib.DateCalculate =
    {
        ...
        getWeekDays: function (selectedDate)
        {
            var currentDay = selectedDate.getDay();
            //if (currentDay == 0) { currentDay = 6 }
            var mondayTime = selectedDate.getTime() - (currentDay) * 24 * 60 * 60 * 1000;
            //var sundayTime = oToday.getTime() + (6 - currentDay) * 24 * 60 * 60 * 1000;
            var mondayDate = new Date(mondayTime)

            var dateReturn = [];
            dateReturn.push(t2v_util.t2vformat.formatdatetime(mondayDate, 'yyyy/mm/dd'));

            for (i = 1; i < 7; i++)
            {
                var tempTick = mondayDate.setDate(mondayDate.getDate() + 1);
                dateReturn.push(t2v_util.t2vformat.formatdatetime(new Date(tempTick), 'yyyy/mm/dd'));
            }
            return dateReturn;
        }
    };
