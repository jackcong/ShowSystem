...
app.factory('DrawTable', ['$http', function ($http) {
    ...
    function DrawTable(divid, colInfo, url,searchparam,pagename,pageindex,showbylink) {

        FetureV7Grid.myParam(colInfo, divid, 20, url, searchparam);
        FetureV7Grid.setParam("currentPageName",pagename);
        FetureV7Grid.setParam("currentPageIndex", pageindex);
        FetureV7Grid.setParam("showByLink", showbylink);
...
var FetureV7Grid =
    {
        ...
        couputePage: function (pageDirection) {
            ...
            if (this.showByLink == true) {
                if (window.location.hash.indexOf('/List/') != -1) {
                    var regPage = /List\/\d+/;

                    var afterReplaceHash = window.location.hash.replace(regPage.exec(window.location.hash), 'List/' + this.currentPageIndex);
                    //add pageindex to current page flag.
                    t2v_StorageData.SavePageSearchCondition(this.currentPageName, JSON.stringify({ "pageindex": this.currentPageIndex }));
                    window.location.href = afterReplaceHash;
                }
            }
            else
            {
                    ...
                    if (inCol[i].disType == "a") {
                        if (inCol[i].isSpecial == "true") {
                            ...
                            if (inCol[i].gridUsedFor != undefined && inCol[i].gridUsedFor != "") {
                                ...
                                $(textTd).append("<a href='javascript:void(0);' class='icon_detail margin-left-3000' onclick='javascript:FetureV7Grid.processFunction($(this)," + inCol[i].onclickFunction + ",\"" + inCol[i].linkparam + "\");'><i class='ace-icon fa fa-pencil blue'></i></a>");
                            }
                        }
                    }
